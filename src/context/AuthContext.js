    ...
    if (condition && !newAchievements[id]) {
      ...
      if (def) {
        ...
        newHistory.push({
          ...
          subtitle: `${def.description} · +${def.reward} баллов`,
          pointsSpent: 0,
...
export const AuthProvider = ({ children }) => {
  ...
  const buySubscription = async (subscriptionId, months = 1, discountPercent = 0) => {
    ...
    if (user.balance < discounted) {
      throw new Error(
        `Недостаточно баллов. Не хватает: ${discounted - user.balance}`
      );
    }
      ...
      history: [
        {
          ...
          subtitle:
            savings > 0
              ? `Списано ${discounted} (скидка ${savings}) · Кэшбэк ${cashback}`
              : `Списано ${discounted} баллов · Кэшбэк ${cashback}`,
