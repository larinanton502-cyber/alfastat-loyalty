...
const matchAutoReply = (text) => {
  ...
  if (/(балл|коин|бонус|кэшбэк|cashback|карт)/.test(t)) {
    return 'α-коины — внутренняя валюта Альфа-карты. Получать их можно за: ежедневный вход (+100), реферальную программу (+2000 за каждого друга), достижения и кэшбэк с покупок (10–15%). Списать α-коины можно при покупке любого тарифа — 1 α-коин = 1 ₽.';
  }
  if (/(промокод|реферал|код|пригласить)/.test(t)) {
    return 'Ваш персональный промокод доступен в Профиле. Поделитесь им с другом — он получит 1000 α-коинов при регистрации, а вы 2000 α-коинов на счёт. Достижение «Друг привёл друга» даст ещё +1500 α-коинов.';
  }
  if (/(пробн|trial|14 дн)/.test(t)) {
    return 'После регистрации вы получаете пробный период 14 дней с активным тарифом «Продвинутый» — все премиум-функции открыты. После окончания тариф можно продлить за α-коины или вернуться на бесплатный план.';
  }
  ...
  if (/(оплат|чек|счёт|деньг|возврат|рефанд)/.test(t)) {
    return 'Оплата в приложении производится α-коинами с Альфа-карты. Для оплаты картой/счётом обратитесь в коммерческий отдел через форму на сайте alfastat.ru. По возвратам и спорным операциям — ответим в течение 24 часов.';
  }
