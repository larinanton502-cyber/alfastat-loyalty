...
const matchAutoReply = (text) => {
  ...
  if (/(балл|бонус|кэшбэк|cashback)/.test(t)) {
    return 'Баллы можно получать за: ежедневный вход (+100), реферальную программу (+2000 за каждого друга), достижения и кэшбэк с покупок (10–15%). Списать баллы можно при покупке любого тарифа — 1 балл = 1 ₽.';
  }
  if (/(промокод|реферал|код|пригласить)/.test(t)) {
    return 'Ваш персональный промокод доступен в Профиле. Поделитесь им с другом — он получит 1000 баллов при регистрации, а вы 2000 баллов на счёт. Достижение «Друг привёл друга» даст ещё +1500 баллов.';
  }
  if (/(пробн|trial|14 дн)/.test(t)) {
    return 'После регистрации вы получаете пробный период 14 дней с активным тарифом «Продвинутый» — все премиум-функции открыты. После окончания тариф можно продлить за баллы или вернуться на бесплатный план.';
  }
  ...
  if (/(оплат|чек|счёт|деньг|возврат|рефанд)/.test(t)) {
    return 'Оплата в приложении производится баллами. Для оплаты картой/счётом обратитесь в коммерческий отдел через форму на сайте alfastat.ru. По возвратам и спорным операциям — ответим в течение 24 часов.';
  }
